    ...
    files.forEach(function(f){
      if (isRawFile(f)) {
        ...
        RawPreview.extract(f, function(err, result){
          ...
          // ====== RAW处理核心流程 ======
          // 重要说明：浏览器对相机RAW预览图的JPEG解码支持有限，
          // 强行用canvas处理常导致输出黑色。因此统一策略：
          //   → 直接提供相机原始JPEG预览图下载
          //   → 不通过canvas做色彩增强（避免黑色输出）
          //   → 如需要增强，建议用户用专业软件（Lightroom等）处理
          var name = f.name.replace(/\.[^.]+$/, '') + '.jpg';
          var rawUrl = result.url || (result.blob ? URL.createObjectURL(result.blob) : null);
          if (!rawUrl) {
            grid.innerHTML += '<p style="color:#ff5050;">' + f.name + '：无法提取预览图</p>';
            finishOne();
            return;
          }
          // 验证图片是否能正常加载显示
          ...
          verifyImg.onload = function(){
            var w = verifyImg.naturalWidth || 0;
            var h = verifyImg.naturalHeight || 0;
            var card = document.createElement('div');
            card.className = 'result-item';
            card.innerHTML =
              '<img class="result-thumb" src="'+rawUrl+'" alt="" style="max-width:100%;max-height:200px;object-fit:contain;border-radius:6px;" />' +
              '<div class="result-info">' +
                '<div class="result-name">'+name+'</div>' +
                '<div class="result-size">'+(result.size/1024).toFixed(1)+' KB | '+(w||'?')+'x'+(h||'?')+' | '+result.source+'</div>' +
                '<a class="result-download" href="'+rawUrl+'" download="'+name+'">⬇ 下载 JPG（相机原始预览）</a>' +
              '</div>';
            grid.appendChild(card);
            finishOne();
          };
          verifyImg.onerror = function(){
            ...
            card.className = 'result-item';
            card.innerHTML =
              '<div style="padding:16px;color:rgba(255,255,255,.5);font-size:13px;">⚠️ 预览图无法直接显示（可能使用了浏览器不支持的色彩空间）</div>' +
              '<div class="result-info">' +
                '<div class="result-name">'+name+'</div>' +
                '<div class="result-size">'+(result.size/1024).toFixed(1)+' KB | '+result.source+'</div>' +
                '<a class="result-download" href="'+rawUrl+'" download="'+name+'">⬇ 下载 JPG（用 Lightroom/Photoshop 打开）</a>' +
              '</div>';
            grid.appendChild(card);
            finishOne();
          };
          verifyImg.src = rawUrl;
        });
      } else {
        // 普通图片
